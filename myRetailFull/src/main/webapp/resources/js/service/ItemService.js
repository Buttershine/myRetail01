...
App.factory('ItemService', ['$http', '$q', function($http, $q){
	...
	return {
			...
			fetchCategoryList: function() {//Fetches category list from server.
				return $http.get('http://localhost:8080/myRetail/categories')
					.then(
								function(response){
			...
			fetchAllItems: function(category) {//Fetches list of item for a specific category.
					return $http.get('http://localhost:8080/myRetail/item/'+category)
							.then(
									function(response){
										return response.data;
									}, 
			...
			fetchSpecificItem: function(category,id) {//Fetches a specific item based on category and item id.
				return $http.get('http://localhost:8080/myRetail/item/'+category+'/'+id)
						.then(
								function(response){
									return response.data;
								}, 
